...
class DatabaseManager:
    ...
    def _get_client(self, uri: str):
        ...
        Uses a lock to ensure we don't create two clients for the same URI.
        """
        # Hash the URI to use as key (never store full URI in memory unnecessarily)
        uri_hash = hash(uri)  # simple hash, but better use a stable hash
        if uri_hash not in self._clients:
            from motor.motor_asyncio import AsyncIOMotorClient
            logger.debug("Creating new Motor client for URI hash %s", uri_hash)
            # Use connectTimeoutMS to avoid hanging
            client = AsyncIOMotorClient(uri, serverSelectionTimeoutMS=5000)
            self._clients[uri_hash] = client
        return self._clients[uri_hash]

    async def get_database(self, uri: str, db_name: str = None):
        """
        Returns a MotorDatabase object for the given URI and optional database name.
        If db_name is None, uses the database from the URI path (if any).
        """
    ...
    async def register_database(self, db_key: str, uri: str, db_name: str = None):
        """
    ...
    async def unregister_database(self, db_key: str):
        """Remove a database from the map (optional cleanup)."""
        async with self._lock:
