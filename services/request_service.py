"""
Request service. Posts to REQST_CHANNEL with token-based buttons.
"""
import logging
...
from core.config import REQST_CHANNEL
...
async def post_request(client: Client, user_id: int, username: str,
    ...
    if not REQST_CHANNEL:
        logger.debug("[REQSVC] REQST_CHANNEL not set")
        return None

    token_id = await request_repo.create_token(
        user_id=user_id, username=username, full_name=full_name,
        movie_name=movie_name, imdb_id=imdb_id, user_query=user_query,
    )
    if not token_id:
        return None
    ...
    try:
        ...
        logger.info(f"[REQSVC] posted token={token_id} movie={movie_name!r}")
        return token_id
